...
regex='\.(cpp|h|hpp|c|cs|py|js|jsx|ts|tsx|java|go|rs|nim|sh|ps1|gd|jsonc|json|pyi|md|ml|sql|ts)$|^[^.]+$'
...
git ls-files | grep -E "$regex" | while IFS= read -r file; do
    ...
    if [[ "$file" != frontend/src/* ]]; then
        case "$file" in
            .vscode/*|test/*|README.md|LICENSE|.gitignore|*.diff|*.json|backend/cmd/seed/main.go|backend/build_lambda.sh)
    ...
    # Exclude infra files if --no-infra
    if $no_infra; then
        case "$file" in
            infra/*)
                continue
