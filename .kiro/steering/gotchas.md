...
`vite-plugin-pwa` is configured with `injectRegisterSW: false` (actually `injectRegister: false`
in v1.x). The SW is registered manually in `main.tsx` after `window.load`. This prevents a
synchronous `<script src="/registerSW.js">` from blocking the initial render (~830ms saving).
Do not re-enable auto-injection.
